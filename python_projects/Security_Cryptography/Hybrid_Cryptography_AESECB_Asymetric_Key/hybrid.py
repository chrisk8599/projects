...
class HybridCipher:
    ...
    def create_aes_cipher(self, length):
        ...
        rsa = PKCS1_OAEP.new(self.remote_pub_key)
        sym_key = rsa.encrypt(sym_len)
    ...
    def decrypt(self, msg):
        rsa = PKCS1_OAEP.new(self.own_key)
        session_key = rsa.decrypt(msg[0])
        aes_cipher = AES.new(session_key, AES.MODE_CBC, msg[2])
        msg_dec = aes_cipher.decrypt(msg[1])
...
def main():
    ...
    b_hybrid_cipher = HybridCipher(32,bob.own_key,alice.get_public_key())
    # Bob decrypts.
